...
switch ($method) {
	case 'GET':
		...
		if(!empty($ID)){
			echo json_encode($controladorContenedor -> buscarContenedor($ID));
			}
		if($uri === '/Proyecto/ProyectoSyntropy/Gestion/miApi/Camiones'){
...
break;
    ...
    if($uri === '/Proyecto/ProyectoSyntropy/Gestion/miApi/RegistrarContenedor'){
			...
			if (!empty($datos->capCarga) && !empty($datos->Tipo) && !empty($datos->Estado) && !empty($datos->Ubicacion)){
			}
			echo json_encode ($controladorContenedor->crearContenedor($datos->capCarga, $datos->Tipo, $datos->Estado, $datos->Ubicacion));
		} else {
			echo json_encode(['error' => 'Faltan campos obligatorios']);
		}
	if($uri === '/Proyecto/ProyectoSyntropy/Gestion/miApi/RegistrarCamion'){
			$json = file_get_contents('php://input');
			$datos = json_decode($json);
